...
The Social SDK itself is native. This adapter keeps the Python service isolated
from the proprietary runtime and talks to the small CYBREX helper over private
stdin/stdout pipes. No localhost listener, OAuth token, Discord user token or
account credential is introduced by this transport.
"""
...
_HELPER_NAMES = (
    'cybrex-discord-social-sdk.exe' if sys.platform == 'win32' else 'cybrex-discord-social-sdk',
)


def discover_social_sdk_helper() -> Optional[Path]:
    """Return a local Social SDK helper path without downloading anything."""
    ...
    for name in _HELPER_NAMES:
        candidates.extend((
            ...
            source_dir / name,
            source_dir / 'bin' / name,
            source_dir / 'native' / 'discord_social_sdk_bridge' / 'build' / name,
            source_dir / 'native' / 'discord_social_sdk_bridge' / 'build' / 'Release' / name,
        ))
...
class SocialSDKPresence:
    """pypresence-like adapter backed by the optional Social SDK helper."""
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise SocialSDKError(f'could not start Discord Social SDK helper: {exc}') from exc
        ...
        try:
            ...
            self._command(encode_command('SET_APP', {'application_id': self.application_id}))
        except Exception:
        ...
        try:
            for line in process.stdout:
                try:
                    ...
                    # A command is strictly request/response; a full queue means
                    # the helper violated the protocol or the caller disappeared.
                    break
    ...
    def _command(self, line: str) -> dict[str, str]:
        with self._command_lock:
            ...
            try:
                ...
                raise SocialSDKError('Discord Social SDK helper returned an invalid response') from exc
            if op == 'OK':
                return fields
            if op == 'ERR':
                code = str(fields.get('code') or 'unknown_error')[:80]
                raise SocialSDKError(f'Discord Social SDK helper rejected command: {code}')
            raise SocialSDKError(f'unexpected Discord Social SDK helper response: {op[:32]}')

    def update(self, **payload: Any) -> None:
        if self._process is None or self._process.poll() is not None:
            self.connect()
        self._command(encode_update(payload))
